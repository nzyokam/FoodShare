...
class _RestaurantDashboardState extends ConsumerState<RestaurantDashboard> {
  ...
  Future<void> _loadProfile() async {
    ...
    final restaurant = await ProfileService.getRestaurant(userId).catchError((_) => null);
    if (mounted && restaurant != null) setState(() => _restaurant = restaurant);
  }
  ...
  Widget _getBody() {
    switch (_selectedIndex) {
      case 0:
        return _buildHome();
      case 1:
        return ChatsListScreen(userType: UserType.restaurant, onDrawerItemSelected: _onNavTap);
      case 2:
        return ProfileScreen(userType: UserType.restaurant, onDrawerItemSelected: _onNavTap);
      case 3:
  ...
  Widget _buildHome() {
    ...
    // Read stats from providers (no separate data load needed)
    final donationsAsync = ref.watch(myDonationsProvider);
    final requestsAsync = ref.watch(receivedRequestsProvider);

    final donations = donationsAsync.asData?.value ?? [];
    final requests = requestsAsync.asData?.value ?? [];

    final totalDonations = donations.length;
    final activeDonations = donations.where((d) => d.status == DonationStatus.available).length;
    final completedDonations = donations.where((d) => d.status == DonationStatus.completed).length;
    final pendingRequests = requests.where((r) => r.status.name == 'pending').length;
              ...
              child: Column(
                ...
                children: [
                  ...
                  _HeroCard(
                    ...
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AddDonationScreen()))
                        .then((_) => ref.invalidate(myDonationsProvider)),
                  ),
                  ...
                  Row(
                    children: [
                      Expanded(child: _ChipCard(icon: Icons.inbox_rounded, label: 'Requests', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DonationRequestsScreen())))),
                  ...
                  const SizedBox(height: 12),
                  _SpotlightStat(
                    iconWidget: const ClipOval(child: AppLogo(width: 28, height: 28)),
                    label: 'Currently Active',
                    value: activeDonations,
                    detail: 'donation${activeDonations == 1 ? '' : 's'} available for pickup',
                  ),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      _MiniStat(label: 'Total\nDonated', value: totalDonations),
                      const SizedBox(width: 8),
                      _MiniStat(label: 'Completed', value: completedDonations),
                      const SizedBox(width: 8),
...
class _SpotlightStat extends StatelessWidget {
  ...
  final int value;
  final String detail;
  const _SpotlightStat({required this.iconWidget, required this.label, required this.value, required this.detail});
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(width: 56, height: 56, decoration: BoxDecoration(color: cs.onPrimaryContainer.withValues(alpha: 0.1), shape: BoxShape.circle), child: iconWidget),
        ],
      ),
    );
  }
